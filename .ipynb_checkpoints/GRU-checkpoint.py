...
class GRUNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, dropout):
        ...
        self.gru = nn.GRU(hidden_dim, hidden_dim, num_layers=n_layers, dropout=dropout)
        self.fc = nn.Linear(hidden_dim, output_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        x = self.dropout(self.embedding(x))
        _, h_n = self.gru(x.permute(1, 0, 2))
        x = h_n[-1]
        x = self.fc(x)
...
# Training the model
model = GRUNet(input_dim=10000, hidden_dim=32, output_dim=1, n_layers=2, dropout=0.5).to(device)
...
def loop_through_ber_values():
    ...
    for ber in ber_values:
        ...
        for setting in settings:
            model_copy = GRUNet(input_dim=10000, hidden_dim=32, output_dim=1, n_layers=2, dropout=0.5).to(device)
