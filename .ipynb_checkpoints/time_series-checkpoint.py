...
settings = [
    #{'q_type': 'signed', 'encode': 'bitmask', 'int_bits': 2, 'frac_bits': 6, 'rep_conf': np.array([2, 2, 2, 2, 2, 2, 2, 2]), 'ber': 0.005},
    #{'q_type': 'unsigned', 'encode': 'bitmask', 'int_bits': 2, 'frac_bits': 6, 'rep_conf': np.array([2, 2, 2, 2, 2, 2, 2, 2]), 'ber': 0.005},
    #{'q_type': 'afloat', 'encode': 'bitmask', 'int_bits': 2, 'frac_bits': 6, 'rep_conf': np.array([2, 2, 2, 2, 2, 2, 2, 2]), 'ber': 0.005},
    {'q_type': 'signed', 'encode': 'dense', 'int_bits': 2, 'frac_bits': 6, 'rep_conf': np.array([2, 2, 2, 2, 2, 2, 2, 2])},
    #{'q_type': 'unsigned', 'encode': 'dense', 'int_bits': 2, 'frac_bits': 6, 'rep_conf': np.array([2, 2, 2, 2, 2, 2, 2, 2]), 'ber': 0.005},
    #{'q_type': 'afloat', 'encode': 'dense', 'int_bits': 2, 'frac_bits': 6, 'rep_conf': np.array([2, 2, 2, 2, 2, 2, 2, 2]), 'ber': 0.005}
]

# List for different BER settings
bers = [0.001, 0.005, 0.01, 0.05, 0.1,0.5,1]
bers = np.logspace(np.log10(10 * 10**-10), np.log10(10 * 10**-2), num=1000)

# Define lists to store Mean Squared Errors and Error Rates for different settings and BER
mse_rates_rnn_ber = []
mse_originals_rnn_ber = []
mse_faultys_rnn_ber = []
mse_rates_lstm_ber = []
mse_originals_lstm_ber = []
mse_faultys_lstm_ber = []

for ber in bers:
    # These lists will hold the error rates for each setting for the current ber
    mse_rates_rnn = []
    mse_originals_rnn = []
    mse_faultys_rnn = []
    mse_rates_lstm = []
    ...
    mse_rates_rnn_ber.append(mse_rates_rnn)
    mse_originals_rnn_ber.append(mse_originals_rnn)
    mse_faultys_rnn_ber.append(mse_faultys_rnn)
    mse_rates_lstm_ber.append(mse_rates_lstm)
    mse_originals_lstm_ber.append(mse_originals_lstm)
    mse_faultys_lstm_ber.append(mse_faultys_lstm)


# Create a dictionary to store the results
results = {
    'original_mse_rnn': mse_original_rnn,
    'original_mse_lstm': mse_original_lstm,
    'settings': [],
}
...
for idx, ber in enumerate(bers):
    ...
    for setting_idx, setting in enumerate(settings):
        setting_results = {
            'setting': f'setting {setting_idx + 1}',
            'error_rate_rnn': mse_rates_rnn_ber[idx][setting_idx],
            'original_mse_rnn': mse_originals_rnn_ber[idx][setting_idx],
            'faulty_mse_rnn': mse_faultys_rnn_ber[idx][setting_idx],
            'error_rate_lstm': mse_rates_lstm_ber[idx][setting_idx],
            'original_mse_lstm': mse_originals_lstm_ber[idx][setting_idx],
            'faulty_mse_lstm': mse_faultys_lstm_ber[idx][setting_idx],
        }
        ber_results['rnn_results'].append(setting_results)
        ber_results['lstm_results'].append(setting_results)
    results['settings'].append(ber_results)

# Save results to a JSON file
with open('results_example_5.json', 'w') as file:
